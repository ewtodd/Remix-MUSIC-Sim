...
//
// Usage: root -l RunMUSICSim_a2p.C
//
...
void RunMUSICSim_a2p()
{
  ...
  // beam
  string beam = "14O";
  string SRIMbeam = SRIMdir + "14O_in_GasIndex3_400Torr_293K.srim";
  // target (no SRIM file, assuming it does not propagate)
  string target = "4He";
  // compound nucleus (no SRIM file, assuming it does not propagate)
  string compound = "18Ne";
  
  // Reac1: 12C(16C,4n2p)22Ne
  // Chan1: 28Mg*->n+27Mg*->n+26Mg*->n+25Mg*->n+24Mg*->p+23Na*->p+22Ne
  const int NumEvapPart = 2;
  string res[NumEvapPart];
  string SRIMres[NumEvapPart];
  string evap[NumEvapPart];
  string SRIMevap[NumEvapPart];
  evap[0] = "p";    SRIMevap[0] = SRIMdir + "1H_in_GasIndex3_400Torr_293K.srim";
  res[0] = "17F";   SRIMres[0] = SRIMdir + "17F_in_GasIndex3_400Torr_293K.srim";
  evap[1] = "p";    SRIMevap[1] = SRIMdir + "1H_in_GasIndex3_400Torr_293K.srim";;
  res[1] = "16O";   SRIMres[1] = SRIMdir + "16O_in_GasIndex3_400Torr_293K.srim";
